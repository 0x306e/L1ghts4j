...
public class UpdateName {
  ...
  // previous ver.
  // public void updateName(Status status, String NewName, String ScreenName)
  ...
  //
  // twitter.updateProfile(NewName, null, null, null);
  // //tweet.ReplyTweet(String.format("名前を\"" + NewName + "\"に変更しました(by @%s)",
  // ScreenName), status);
  // twitter.updateStatus(new StatusUpdate(String.format("名前を\"" + NewName +
  // "\"に変更しました(by @%s)", ScreenName))
  // .inReplyToStatusId(status.getId()));
  // logger.ouputUpdateNameLog(String.format("Name has changed to \"%s\" by
  // @%s", NewName, ScreenName));
  // System.out.println(String.format("Name has changed to \"%s\" by @%s",
  // NewName, ScreenName));
  //
  ...
  // "\"に変更しました(by @%s)", ScreenName))
  // .inReplyToStatusId(status.getId()));
  // //tweet.ReplyTweet(String.format("@%s 20文字以上には設定できません", ScreenName),
  // status);
  // logger.ouputErrorLog(String.format(
  // "Tried to change the name to \"%s\" by @%s, but occurred Out of length
  // error.", NewName, ScreenName));
  // System.out.println(String.format("Tried to change the name to \"%s\" by
  // @%s, but occurred Out of length error.",
  // NewName, ScreenName));
  //
  // } else if (NewName.indexOf("Twitter") != -1)
  // {
  //
  // twitter.updateStatus(new StatusUpdate(String.format("@%s
  // \"Twitter\"を含む名前には設定できません", ScreenName))
  // .inReplyToStatusId(status.getId()));
  // //tweet.ReplyTweet(String.format("@%s \"Twitter\"を含む名前には設定できません",
  // ScreenName), status);
  // logger.ouputErrorLog(
  // String.format("Tried to change the name to \"%s\" by @%s, but can't change
  // to include strings \"Twitter\".",
  // NewName, ScreenName));
  // System.out.println(
  // String.format("Tried to change the name to \"%s\" by @%s, but can't change
  // to include strings \"Twitter\".",
  ...
  // {
  //
  // twitter.updateStatus(new StatusUpdate(String.format("@%s 名前を空にはできません",
  // ScreenName))
  // .inReplyToStatusId(status.getId()));
  // //tweet.ReplyTweet(String.format("@%s 名前を空にはできません", ScreenName), status);
  // logger.ouputErrorLog(
  // String.format("Tried to change the name to \"%s\" by @%s, but can't change
  // to null.", NewName, ScreenName));
  // System.out.println(
  // String.format("Tried to change the name to \"%s\" by @%s, but can't change
  // to null.", NewName, ScreenName));
  //
  ...
  // {
  //
  // twitter.updateStatus(new StatusUpdate(String.format("@%s 不明なエラーが発生しました",
  // ScreenName))
  // .inReplyToStatusId(status.getId()));
