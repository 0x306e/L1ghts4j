    ...
    if (settingReader.getUpdateNameAccessLevel() == 2) {
      try {
        ...
        if (newLocation.length() > 30) {
          ...
          logger
              .ouputErrorLog("Location has couldn't changed to " + newLocation + " (Unknown error)(by " + screenName + ")");
        }
        e.printStackTrace();
      }
      ...
      logger
          .ouputErrorLog("Loction has couldn't changed to " + newLocation + " (Permission denied)(by " + screenName + ")");
