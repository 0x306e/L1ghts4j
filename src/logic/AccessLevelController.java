...
import static util.SettingReader.getUpdateLocationAccessLevel;
import static util.SettingReader.getUpdateNameAccessLevel;
import static util.SettingWriter.setUpdateLocationAccessLevel;
import static util.SettingWriter.setUpdateNameAccessLevel;
...
public class AccessLevelController extends TwitterHeader {
    ...
    public AccessLevelController(Twitter t) {
        twitter = t;
        tweet = new Tweet(t);
    }
    ...
    public void setUpdateNameLevel(Status status) throws IllegalStateException, TwitterException {
        ...
        if (status.getText().contains(twitter.getScreenName()) && status.getText().contains("set-level update_name")) {
            if (!twitter.getScreenName().equals(status.getUser().getScreenName())) {
                try {
                    setUpdateNameAccessLevel(
                            status.getText().substring(status.getText().indexOf("update_name") + 11).trim());
                } catch (IllegalArgumentException e) {
                    tweet.ReplyTweet(
                            String.format("@%s 値は0~2で指定してください。" + TIME_HEADER, status.getUser().getScreenName()),
                            status);
                    e.printStackTrace();
                }
            } else {
                tweet.ReplyTweet(
                        String.format("@%s Permission denied." + TIME_HEADER, status.getUser().getScreenName()),
                        status);
            }
        }
    }
    ...
    public boolean getUpdateLocationLevel(Status status) throws IllegalStateException, TwitterException {
        ...
        if (status.getText().contains(twitter.getScreenName())
                && status.getText().contains("get-level update_location")) {
    ...
    public void setUpdateLocationLevel(Status status) throws IllegalStateException, TwitterException {
        ...
        if (status.getText().contains(twitter.getScreenName())
                && status.getText().contains("set-level update_location")) {
            if (!twitter.getScreenName().equals(status.getUser().getScreenName())) {
                try {
                    setUpdateLocationAccessLevel(
                            status.getText().substring(status.getText().indexOf("update_name") + 11).trim());
                } catch (IllegalArgumentException e) {
                    tweet.ReplyTweet(
                            String.format("@%s 値は0~2で指定してください。" + TIME_HEADER, status.getUser().getScreenName()),
